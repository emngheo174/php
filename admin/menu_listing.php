...
if (!empty($_SESSION['current_user'])) {
    ...
    if (!empty($where)) {
        $currentMenu = mysqli_query($con, "SELECT * FROM `menu1`  where (" . $where . ")");
        $currentMenu = mysqli_fetch_assoc($currentMenu);
        if (!empty($currentMenu)) {
            $menuList = mysqli_query($con, "SELECT * FROM `menu1` ORDER BY `menu`.`position` ASC"); //Lấy tất cả các menu
        ...
        $menu = mysqli_query($con, "SELECT * FROM `menu1` ORDER BY `menu1`.`position` ASC");
