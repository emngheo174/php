    ...
    <div class="main-content">
        ...
        <div id="content-box">
            ...
            if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['link']) && !empty($_POST['link'])) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại danh mục                            
                            $result = mysqli_query($con, "UPDATE `menu1` SET `name` = '".$_POST['name']."', `link` = '".$_POST['link']."', `parent_id` = '".$_POST['parent_id']."', `position` = '".$_POST['position']."', `last_updated` = '".time()."' WHERE `menu1`.`id` = ".$_GET['id'].";");
                        } else { //Thêm danh mục
                            $result = mysqli_query($con, "INSERT INTO `menu1` (`id`, `parent_id`, `name`, `link`, `position`, `created_time`, `last_updated`) VALUES (NULL, '".$_POST['parent_id']."', '".$_POST['name']."', '".$_POST['link']."', '".$_POST['position']."', ".time().", ".time().");");
                        }
                ...
                $result = mysqli_query($con, "SELECT * FROM `menu1` ORDER BY `menu1`.`position` ASC");
                $menuList = mysqli_fetch_all($result, MYSQLI_ASSOC);
                $menuTree = createMenuTree($menuList, 0);
                //Sửa danh mục
                if (!empty($_GET['id'])) {
                    $result = mysqli_query($con, "SELECT * FROM `menu1` WHERE `id` = " . $_GET['id']);
